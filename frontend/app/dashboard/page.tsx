...
export default function DashboardPage() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    if (result.success && result.data) {
      ...
      alert("🥦 Plan ishrane uspešno generisan preko Mifflin-St Jeor jednačine i alociran u bazu!");
    } else {
    ...
    <div className="min-h-screen bg-slate-950 p-6 text-white">
      <div className="max-w-6xl mx-auto space-y-8">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Generisanje personalizovanih planova ishrane u realnom vremenu.</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* FORMUL_AR ZA UNOS ZDRAVSTVENIH METRIKA */}
          <form onSubmit={handleGenerate} className="lg:col-span-1 p-6 bg-slate-900 border border-slate-800 rounded-2xl space-y-4">
              ...
              plans.map((plan) => (
                <div key={plan.id} className="p-5 bg-slate-900 border border-slate-800 rounded-xl space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2 text-xs text-center text-slate-400">
                    ...
                    <div className="bg-slate-950 p-2 rounded">🍎 Užina: {plan.meals.snacks?.join(", ") || "Voće/Orašasti plodovi"}</div>
                  </div>
                </div>
              ))
            )}
